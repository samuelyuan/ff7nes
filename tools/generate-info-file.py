...
def generateInfoFileForStringBank(nesFile, bankNum, stringCounts, bankStringBlockName, endAllTextAddr):
    bankNumStr = convertToHex(bankNum).zfill(2)
    ...
    boundaries = [
        (0x8000, "ADDRTABLE"),
        (0x8000 + (len(stringCounts) * 2), "BYTETABLE"),
    ]
    startAddrArr = nesFile.getAddressBlock(bankNum, 0x00, 0x00 + (len(stringCounts) * 2))

    labels = []
    for i in range(len(startAddrArr)):
        labels.append((startAddrArr[i], bankStringBlockName[i]))

    for i in range(len(stringCounts)):
        boundaries.extend([
            (startAddrArr[i], "ADDRTABLE"),
            (startAddrArr[i] + stringCounts[i] * 2, "TEXTTABLE"),
        ])

        currentBlockAddrArr = nesFile.getAddressBlockAll(bankNum, startAddrArr[i] - 0x8000, startAddrArr[i] + stringCounts[i] * 2 - 0x8000)

        addrSet = set()
        for j, addr in enumerate(currentBlockAddrArr):
            if addr in addrSet or addr < 0x8000:
                continue
            labelName = f"{bankStringBlockName[i]}Index{j}"
            labels.append((addr, labelName))
            addrSet.add(addr)

    # end of all text
    boundaries.append((endAllTextAddr, "BYTETABLE"))
...
def generateInfoFileStringBank7(nesFile):
    ...
    endAllTextAddr = 0xFFB8
    generateInfoFileForStringBank(nesFile, 0x07, stringCounts, bankStringBlockName, endAllTextAddr)

def generateInfoFileStringBank0A(nesFile):
    stringCounts = [255, 199, 37]
    bankStringBlockName = [
        "Bank0aDialogueBlock0",
        "Bank0aDialogueBlock1",
        "LocationNameText",
    ]
    endAllTextAddr = 0xED34
    generateInfoFileForStringBank(nesFile, 0x0A, stringCounts, bankStringBlockName, endAllTextAddr)

def generateInfoFileStringBank0B(nesFile):
    stringCounts = [254, 252, 14]
    bankStringBlockName = [
        "Bank0bDialogueBlock0",
        "Bank0bDialogueBlock1",
        "Bank0bDialogueBlock2",
    ]
    endAllTextAddr = 0xFFB0
    generateInfoFileForStringBank(nesFile, 0x0B, stringCounts, bankStringBlockName, endAllTextAddr)

def generateInfoFileStringBank0C(nesFile):
    stringCounts = [251, 164]
    bankStringBlockName = [
        "Bank0cDialogueBlock0",
        "Bank0cDialogueBlock1",
    ]
    endAllTextAddr = 0xF6CF
    generateInfoFileForStringBank(nesFile, 0x0C, stringCounts, bankStringBlockName, endAllTextAddr)

def generateInfoFileStringBank0D(nesFile):
    stringCounts = [255, 178, 117]
    bankStringBlockName = [
        "Bank0dDialogueBlock0",
        "Bank0dDialogueBlock1",
        "Bank0dDialogueBlock2",
    ]
    endAllTextAddr = 0xFFC0
    generateInfoFileForStringBank(nesFile, 0x0D, stringCounts, bankStringBlockName, endAllTextAddr)


def generateInfoFileStringBank0F(nesFile):
    stringCounts = [252, 247]
    bankStringBlockName = [
        "Bank0fDialogueBlock0",
        "Bank0fDialogueBlock1",
        "Bank0fDialogueBlock2",
    ]
    endAllTextAddr = 0xFFC0
    generateInfoFileForStringBank(nesFile, 0x0F, stringCounts, bankStringBlockName, endAllTextAddr)
